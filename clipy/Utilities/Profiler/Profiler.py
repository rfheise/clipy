...
class Profile():
    ...
    def summary(self):
        if self.elapsed_time is None:
            Logger.log_error(f"Profile {self.name} not stopped properly")
            return 
        minutes = int(self.elapsed_time//60)
        seconds = int(self.elapsed_time % 60)
        Logger.new_line()
        Logger.log(f"Profile {self.name} took {minutes}:{seconds} minutes")
        Logger.new_line()
...
class Profiler():
    ...
    def stop(name=None):
        ...
        if name not in Profiler.profiles:
            Logger.log_error(f"Profile {name} not started")
            return
